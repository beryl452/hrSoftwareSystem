...
const TableTransfer = ({ }) => {
  ...
  const [seeTransfer, setSeeTransfer] = React.useState(false);
  ...
  async function viewTranfers() {
    const response = await http.get("/api/transfer/transfer");
    console.log("Transfer", response.data.transfers);
    setTransfers(response.data.transfers);
    console.log(transfers);
  }
  useEffect(() => {
    viewTranfers();
    setSeeTransfer(!seeTransfer);
  }, []);
  return (
          ...
          (success) && (<div className="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-4">
            ...
            <div className="w-full">
              <h5 className="mb-3 text-lg font-semibold text-black dark:text-[#34D399] ">
                Demande de transfer validée avec Succès
              </h5>
              ...
              <input
                ...
                onChange={
                  async (e) => {
                    ...
                    const url = "api/transfer/allAbsences?search=".concat(search);
                    console.log("url =", url);
                    const response = await http.get(url);
                    console.log("search =", response.data);
                    console.log("search =", response.data);
                    setTransfers(response.data);
                    console.log("seacdcdrch =", transfers);
                  }
                }
              />
            </div>
              ...
              {transfers ? (
                transfers.data?.map((transfer, index) => (
                  <tr
                    ...
                    onClick={() => {
                      // navigate(`/transfers/${transfer.id}/contracts`, { state: { transfer: transfer } }, {replace: true})
                    }}
                  >
                    <th
                      scope="row"
                    ...
                    onChange={async () => {
                        // const url = "api/transfer/validate/".concat(transfer.id);
                        await http.put(`api/transfer/approuve/${transfer.id}`)
                                                    .then((response)=>{
                                                      setTransfers(response.data.transfers);
                                                      setSuccess(true);
                                                      console.log("response.data.transfers =", response.data);
                                                    });
                      }
                    }
                      checked={transfer.approuve} />
                    </td>
                    <td className="px-4 py-3">
                    <div className="flex justify-start">
                        ...
                        onClick={
                          async () => {
                            const response = await http.delete(`api/transfer/delete/${transfer.id}`);
        ...
        >
          <span className="text-gray-500 dark:text-gray-400 text-sm font-medium">
            ...
            <span className="text-gray-900 font-medium dark:text-white">
              {' '.concat((transfers.meta ? transfers.meta.from : 0), ' ')} - {' '.concat((transfers.meta ? transfers.meta.to : 0), ' ')}
            </span>
            of
            <span className="text-gray-900 font-medium dark:text-white">
              {' '.concat((transfers.meta ? transfers.meta.total : 0), ' ')}
            </span>
          </span>
          ...
          <ul className="inline-flex items-stretch  -space-x-px dark:border-strokedark dark:bg-boxdark">
            {transfers ? (
              transfers.meta ? (
                transfers.meta.links.length > 3 && (
                  <>
                    {transfers.meta.links.map((link, key) =>
                      link.label === '&laquo; Previous' ? (
                        <li key={key}>
                          <a
                            onClick={async () => {
                              if (transfers.links.prev != null) {
